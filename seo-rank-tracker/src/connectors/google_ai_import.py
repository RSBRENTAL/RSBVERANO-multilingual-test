...
def run(dry_run=False):
    ...
    for path in files:
        with path.open(newline="", encoding="utf-8") as handle:
            ...
            for item in reader:
                rows.append(Result(date=item.get("date", ""), source="google_search_console", engine="google", surface="google_generative_ai", endpoint=item.get("report_type", "manual_csv_import"), country=item.get("country", ""), device=item.get("device", ""), impressions=item.get("impressions", ""), url=item.get("page") or item.get("url", ""), ai_feature_present=explicit_ai_present(item), brand_mentioned=item.get("brand_mentioned", ""), domain_cited=item.get("domain_cited", ""), citation_url=item.get("citation_url", ""), status="ok", error=("Unknown columns: " + ",".join(unknown)) if unknown else "").to_row())
