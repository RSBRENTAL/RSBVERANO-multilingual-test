...
def run(dry_run=False):
    ...
    if dry_run:
        return [Result(source="google_search_console", engine="google", surface="google_generative_ai", status="ok", error=f"dry-run: {len(files)} import files found").to_row()]
    rows=[]
    for path in files:
